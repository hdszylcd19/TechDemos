 ...
 * @author OneD
 * @version 1.0
 * @since 2024/11/6 13:44
 */
object RemoveElement : BaseAlgorithmTest() {
    @JvmStatic
    fun main(args: Array<String>) {
        val nums =
            NumberUtils.generateRandomIntArray(min = 0, max = 50, length = (0..100).random())
        val target = NumberUtils.generateRandomInt(0, 100)
        println("nums: ${nums.contentToString()}, target: $target")
        println("result: ${nums.contentToString()}, ${removeElement(nums, target)}")
    }

    fun removeElement(nums: IntArray, target: Int): Int {
        var k = 0 //永远指向最后一个不为target的索引位置
